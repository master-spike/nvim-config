; extends
; Three things upstream's query does not provide for mini.ai:
;
...
; 3. @statement.outer: upstream defines @statement.inner but no
;    statement.outer, so `ad` has nothing to select.
;
